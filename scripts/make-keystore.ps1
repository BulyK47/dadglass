...
# Read it straight back with the password still in hand. Doing this here, and
# not after the passwords have been cleared, keeps the check honest: an earlier
# version dug the password back out of the properties array by index and only
# ever proved that the index was wrong.
...
# Build every value BEFORE the array literal. Inside @(...), an element written
# as  "prefix=" + ($x -replace 'a','b')  is not one string: PowerShell splits it
# into two elements, which silently puts the path on its own line and leaves
# storeFile empty. Assign first, interpolate second.
...
# Guard against the properties file being subtly malformed: Gradle would then
# fail much later with an error that says nothing about this file.
